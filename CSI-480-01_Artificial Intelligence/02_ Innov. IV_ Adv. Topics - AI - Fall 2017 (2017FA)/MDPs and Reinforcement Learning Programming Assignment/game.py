...
class Game:
    ...
    def __init__(self, agents, display, rules, starting_index=0, mute_agents=False, catch_exceptions=False):
        ...
        import cStringIO
        self.agent_output = [cStringIO.StringIO() for agent in agents]
            ...
            if self.catch_exceptions:
                try:
                    ...
                    try:
                        ...
                        print >> sys.stderr, "Agent %d timed out on a single move!" % agent_index
                        self.agent_timeout = True
                    ...
                    if move_time > self.rules.get_move_warning_time(agent_index):
                        ...
                        print >> sys.stderr, "Agent %d took too long to make a move! This is warning %d" % (agent_index, self.total_agent_time_warnings[agent_index])
                        if self.total_agent_time_warnings[agent_index] > self.rules.get_max_time_warnings(agent_index):
                            print >> sys.stderr, "Agent %d exceeded the maximum number of warnings: %d" % (agent_index, self.total_agent_time_warnings[agent_index])
                            self.agent_timeout = True
                            self._agent_crash(agent_index, quiet=True)
                            self.unmute()
                            return
                    ...
                    if self.total_agent_times[agent_index] > self.rules.get_max_total_time(agent_index):
                        print >> sys.stderr, "Agent %d ran out of time! (time: %1.2f)" % (agent_index, self.total_agent_times[agent_index])
                        self.agent_timeout = True
                        self._agent_crash(agent_index, quiet=True)
                        self.unmute()
                        return
