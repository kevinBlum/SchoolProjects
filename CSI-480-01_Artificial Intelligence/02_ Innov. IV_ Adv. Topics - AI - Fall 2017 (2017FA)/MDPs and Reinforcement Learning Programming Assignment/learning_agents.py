    ...
    def final(self, state):
        ...
        if self.episodes_so_far % NUM_EPS_UPDATE == 0:
            ...
            if self.episodes_so_far <= self.num_training:
                ...
                print '\t_completed %d out of %d training episodes' % (
                       self.episodes_so_far, self.num_training)
                print '\t_average Rewards over all training: %.2f' % (
                        train_avg)
            else:
                test_avg = float(self.accum_test_rewards) / (self.episodes_so_far - self.num_training)
                print '\t_completed %d test episodes' % (self.episodes_so_far - self.num_training)
                print '\t_average Rewards over testing: %.2f' % test_avg
            print '\t_average Rewards for last %d episodes: %.2f' % (
                    NUM_EPS_UPDATE, window_avg)
            print '\t_episode took %.2f seconds' % (time.time() - self.episode_start_time)
            self.last_window_accum_rewards = 0.0
            self.episode_start_time = time.time()
