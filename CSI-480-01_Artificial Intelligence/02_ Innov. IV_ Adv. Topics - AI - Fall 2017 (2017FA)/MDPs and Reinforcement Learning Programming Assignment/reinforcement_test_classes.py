...
class ValueIterationTest(test_classes.TestCase):
    ...
    def __init__(self, question, test_dict):
        ...
        self.nums_iterations_for_display = range(min(iterations, max_pre_iterations))
...
class ApproximateQLearningTest(test_classes.TestCase):
    ...
    def __init__(self, question, test_dict):
        ...
        self.nums_experiences_for_display = range(min(num_experiences, max_pre_experiences))
        self.test_out_file = test_dict['test_out_file']
    ...
    def execute_n_experiences(self, grades, module_dict, solution_dict, n):
        ...
        if last_experience is not None:
            file_out_string += "Agent observed the transition (start_state = %s, action = %s, end_state = %s, reward = %f)\n\n" % last_experience
    ...
    def run_agent(self, module_dict, num_experiences):
        ...
        states = filter(lambda state: len(self.grid.get_possible_actions(state)) > 0, self.grid.get_states())
        states.sort()
        ...
        for i in range(num_experiences):
            ...
            action = rand_obj.choice(self.grid.get_possible_actions(start_state))
            (end_state, reward) = self.env.get_random_next_state(start_state, action, rand_obj=rand_obj)
...
### q6
class Question6Test(test_classes.TestCase):

    def __init__(self, question, test_dict):
        super(Question6Test, self).__init__(question, test_dict)
    ...
    for row in grid:
        for x, col in enumerate(row):
            ...
            if col == "_":
                col = ' '
            row[x] = col
    return grid_world.make_grid(grid)
...
class GridPolicyTest(test_classes.TestCase):
    ...
    def execute(self, grades, module_dict, solution_dict):
        ...
        if not policy_passed:
            ...
            self.print_grid_world()
            return self.test_fail(grades)

        ## check path
        path = follow_path(policy, self.grid.get_start_state())

        if self.path_visits != None and self.path_visits not in path:
        ...
        if self.path_not_visits != None and self.path_not_visits in path:
