...
def read_command(argv):
    ...
    parser.set_defaults(generate_solutions=False, edx_output=False, mute_output=False, print_test_case=False, no_graphics=False)
...
def load_module_string(module_source):
    ...
    exec module_code_dict[k] in tmp.__dict__
...
def evaluate(generate_solutions, test_root, module_dict, exception_map=ERROR_HINT_MAP, edx_output=False, mute_output=False,
    ...
    grades = grading.Grades(project_params.PROJECT_NAME, questions, edx_output=edx_output, mute_output=mute_output)
    if question_to_grade == None:
