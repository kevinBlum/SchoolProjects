...
def parse_tree_problem(test_dict):
    ...
    for line in test_dict["evaluation"].split('\n'):
        ...
        if len(tokens) == 2:
            ...
            raise Exception, "[parse_tree] Bad evaluation line: |%s|" % (line,)
    ...
    for line in test_dict["successors"].split('\n'):
        ...
        if len(tokens) == 3:
            ...
            raise Exception, "[parse_tree] Bad successor line: |%s|" % (line,)
...
class GradingAgent(Agent):
    ...
    def get_action(self, state):
        ...
        if not current_right_states_explored and self.wrong_states_explored < 0:
            self.wrong_states_explored = 1
...
class PacmanGameTreeTest(test_classes.TestCase):
    ...
    def execute(self, grades, module_dict, solution_dict):
        ...
        all_actions = map(lambda x: json.loads(x), solution_dict['optimal_actions'].split('\n'))
        alt_depth_actions = map(lambda x: json.loads(x), solution_dict['alt_depth_actions'].split('\n'))
        partial_ply_bug_actions = map(lambda x: json.loads(x), solution_dict['partial_ply_bug_actions'].split('\n'))
        ...
        if code == 0:
            ...
            self.add_message('State:%s\n_student Move:%s\n_optimal Move:%s' % (state, student_move, opt_move))
            return self.test_fail(grades)
