...
search_agents.py solves the problems for 5,6,7,8
It implements the functions necessary to solve the corners problems and to solve
the food heuristic problem as well as the closest dot search problem
...
class CornersProblem(search.SearchProblem):
    ...
    def get_start_state(self):
        ...
        "*** YOUR CODE HERE ***"
        """
        corners is a tuple of booleans initialized to false
        start state is tuple of the starting position and the corners tuple 
        """
        corners = (False, False, False, False)
        start = (self.starting_position, corners)
        return start
    ...
    def is_goal_state(self, state):
        ...
        "*** YOUR CODE HERE ***"
        """
        corners is set to the state that is passed into function
        boolean is boolean value of the corners and their boolean value logically added together
        then returns boolean and if true then goal state is true
        """
        corners = state[1]
    ...
    def get_successors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            "*** YOUR CODE HERE ***"
            """
            starts with the given code where current position is the state at 0 index
            if statement checks if the next state will hit a wall, then checks the next state
            for each corner and creates a new list of corners based on the boolean value of
            the corners at the next state.
            the successor becomes the next state value and the new corners plus
            else the successor becoems the next state with the original corners value
            """
            x,y = state[0]
            dx, dy = Actions.direction_to_vector(action)
            nextx, nexty = int(x + dx), int(y + dy)
            ...
            if not hits_wall:
                if nextState in self.corners:
                    if nextState == (self.right, 1):
                        newCorners = [True, cornersTest[1], cornersTest[2], cornersTest[3]]
                    elif nextState == (self.right, self.top):
                        newCorners = [cornersTest[0], True, cornersTest[2], cornersTest[3]]
                    elif nextState == (1, self.top):
                        newCorners = [cornersTest[0], cornersTest[1], True, cornersTest[3]]
                    elif nextState == (1,1):
                        newCorners = [cornersTest[0], cornersTest[1], cornersTest[2], True]
                    successor = ((nextState, newCorners), action,  1)
                else:
                    successor = ((nextState, cornersTest), action, 1)
...
def corners_heuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    """
    Define the corners based on the state given
    and find the corners not in the state
    """
    ...
    for corner in corners:
        if corner == (1,1):
            if not cornersNext[3]:
                cornerNot.append(corner)
        if corner == (1, top):
            if not cornersNext[2]:
                cornerNot.append(corner)
        if corner == (right, top):
            if not cornersNext[1]:
                cornerNot.append(corner)
        if corner == (right, 1):
            if not cornersNext[0]:
                cornerNot.append(corner)
    """
    Using the array of corner not found
    main loop to find the heuristic to the corners
    while there are values in the cornerNot array
    then find the distance for each value in the array by using the
    manhattan distance heuristic. After finding all the distance values
    find the minumum distance in the distanceValues array and add to the cost
    then index the minimum distance and set the current positon to the corner and its
    index of the distance then remove the minumum distance and repeat until cornerNot is empty
    """
    cost = 0
    while len(cornerNot) > 0:
        distanceValues = []
        x = 0
        for cornerVal in range(0, len(cornerNot)):
            corner = cornerNot[cornerVal]
            x = x + 1
            distance = util.manhattan_distance(current_position, corner)
            distanceValues.append(distance)
        minimumDistance = min(distanceValues)
        cost += minimumDistance
        minimumDistanceIndex = distanceValues.index(minimumDistance)
        current_position = cornerNot[minimumDistanceIndex]
        del cornerNot[minimumDistanceIndex]
...
def food_heuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    """
    Use food_grid.as_list() to get coordinates of food
    First check if foodlist is empty if so return 0 because no food
    and then check if foodlist has only 1 value, if so then return the distance
    to that food
    """
    foodlist = food_grid.as_list()
    if not foodlist:
        return 0
    oneFood = foodlist[0]
    if len(foodlist) == 1:
        return abs(position[0] - oneFood[0]) + abs(position[1] - oneFood[1])
    """
    For any amount of food greater than 1 find the cost of collecting food in
    the list of food based on the coordinate values relating to the current position
    once the minimum cost from the position to the food is found calculate the high
    cost using the food value and its relation to the minimumFood list
    Then calculate the heuristic based on the coordinates from the two lists created and the
    current position
    """
    cost = 0
    costValues = []
    updateCostValues = []
    minimumFoodCost = 0
    for foodValue in foodlist:
        cost = abs(position[0] - foodValue[0]) + abs(position[1] - foodValue[1])
        costValues.append(cost)
        minimumFoodCost = min(costValues)
    minimumFoodIndex = costValues.index(minimumFoodCost)
    minimumFood = foodlist[minimumFoodIndex]
    for foodValue in foodlist:
        cost = abs(minimumFood[0] - foodValue[0]) + abs(minimumFood[1] - foodValue[1])
        updateCostValues.append(cost)
        highCost = max(updateCostValues)
    maximumIndex = updateCostValues.index(highCost)
    maximumFood = foodlist[maximumIndex]
    return abs(minimumFood[0] - maximumFood[0]) + abs(minimumFood[1] - maximumFood[1]) + abs(position[0] - minimumFood[0]) + abs(position[1] - minimumFood[1])
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def is_goal_state(self, state):
        ...
        "*** YOUR CODE HERE ***"
        """
        Set food equal to the food state
        if true then yes it is a goal state
        otherwise it is not a goal state
        """
        food = self.food
        if (food[x][y] == True):
            return True
